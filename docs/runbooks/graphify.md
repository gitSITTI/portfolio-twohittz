# Runbook — Graphify (portfolio-twohittz)
...
```bash
cd /home/user/portfolio-twohittz
graphify scan .
```
...
If OB1 is up, a capture event should appear in OB1's `captures` table with `kind="graph"` and `metadata.source_repo="portfolio-twohittz"`.
